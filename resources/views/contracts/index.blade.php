                        ...
                        <div class="card-luxury p-6 flex flex-col md:flex-row items-center gap-6 group hover:border-primary/50 transition-all">
                            ...
                            <div class="flex-grow space-y-2">
                                <div class="flex items-center justify-between">
                                    ...
                                    @php
                                        $statusConfig = [
                                            'pending' => ['label' => 'Chờ xử lý', 'class' => 'bg-amber-500/10 text-amber-500'],
                                            'confirmed' => ['label' => 'Đã xác nhận', 'class' => 'bg-blue-500/10 text-blue-500'],
                                            'active' => ['label' => 'Đang thực hiện', 'class' => 'bg-indigo-500/10 text-indigo-500'],
                                            'complete' => ['label' => 'Hoàn thành', 'class' => 'bg-green-500/10 text-green-500'],
                                            'cancelled' => ['label' => 'Đã hủy', 'class' => 'bg-red-500/10 text-red-500'],
                                            'signed' => ['label' => 'Đã ký kết', 'class' => 'bg-green-500/10 text-green-500'],
                                        ];
                                        $status = $statusConfig[$contract->status] ?? ['label' => $contract->status, 'class' => 'bg-muted text-muted-foreground'];
                                    @endphp
                                    <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase {{ $status['class'] }}">
                                        {{ $status['label'] }}
                                    </span>
                                </div>
